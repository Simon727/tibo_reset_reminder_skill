...
2. **安静监控**：之后每 5 分钟运行一次，没有新的明确信息就保持静默。
...
Skill 遵循通用的 Agent Skills 目录格式。能否持续每 5 分钟运行，取决于宿主是否提供 Automation、Scheduled Task 或 Cronjob。
...
```mermaid
flowchart LR
    A["QStash 每 5 分钟刷新"] --> B["Vercel 公共 Feed"]
